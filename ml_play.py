...
class MLPlay:
    def __init__(self, side):
        ...
        if self.side == "1P":
            with open(os.path.join(os.path.dirname(__file__),'save','1Pmodel.pickle'), 'rb') as f:
                self.model = pickle.load(f)
        else:
            with open(os.path.join(os.path.dirname(__file__),'save','2Pmodel1.pickle'), 'rb') as f:
                self.model = pickle.load(f)
